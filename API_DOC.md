...
|rlGetMatrixProjectionStereo |**eye**=0 |Get internal projection matrix for stereo render (selected eye) |
|rlGetMatrixViewOffsetStereo |**eye**=0 |Get internal view offset matrix for stereo render (selected eye) |
|rlSetMatrixProjectionStereo |**right**, **left** |Set eyes projection matrices for stereo rendering |
|rlSetMatrixViewOffsetStereo |**right**, **left** |Set eyes view offsets matrices for stereo rendering |
|rlBegin |**mode**=RL_TRIANGLES |Initialize drawing mode (how to organize vertex) |
|rlEnd | |Finish vertex providing |
|rlVertex2i |**x**=0, **y**=0 |Define one vertex (position) |
|rlVertex2f |**x**=0, **y**=0 |Define one vertex (position) |
|rlVertex3f |**x**=0, **y**=0, **z**=0 |Define one vertex (position) NOTE: Vertex position data is the basic information required for drawing |
|rlTexCoord2f |**x**=0, **y**=0 |Define one vertex (texture coordinate) NOTE: Texture coordinates are limited to QUADS only |
|rlNormal3f |**x**=0, **y**=0, **z**=1 |Define one vertex (normal) NOTE: Normals limited to TRIANGLES only? |
|rlColor4ub |**r**=255, **g**=255, **b**=255, **a**=255 |Define one vertex (color) |
|rlColor3f |**x**=1, **y**=1, **z**=1 |Define one vertex (color) |
|rlColor4f |**x**=1, **y**=1, **z**=1, **w**=1 |Define one vertex (color) |
|rlSetTexture |**id**=0 |Set current texture to use |
|rlCheckRenderBatchLimit |**vCount** |Check internal buffer overflow for a given number of vertex and force a rlRenderBatch draw call if required |
|rlEnableColorBlend | | |
|rlDisableColorBlend | | |
|rlColorMask |**r**=1, **g**=1, **b**=1, **a**=1 | |
|rlSetCullFace |**mode**=RL_CULL_FACE_BACK |Set face culling mode |
|rlEnableScissorTest | | |
...
|rlClearColor |**r**=0, **g**=0, **b**=0, **a**=255 |Clear color buffer with color |
|rlClearScreenBuffers | |Clear used screen buffers (color and depth) |
|rlCheckErrors | |Check and log OpenGL error codes |
|rlSetBlendMode |**mode**=RL_BLEND_ALPHA |Set blend mode |
|rlLoadDrawCube | |Load and draw a cube in NDC |
|rlLoadDrawQuad | |Load and draw a quad in NDC |
|rlGetVersion | |Get current OpenGL version |
|rlSetFramebufferWidth |**width** |Set current framebuffer width |
|rlGetFramebufferWidth | |Get default framebuffer width |
|rlSetFramebufferHeight |**height** |Set current framebuffer height |
...
|MeasureTextCodepoints |**font**, **codepoints**, **fontSize**=20, **spacing**=0 |Measure string size for an existing array of codepoints for Font |
...
|TextSubtext |**text**, **position**=0, **length**=0 | |
...
|LoadTextLines |**text** | |
|UnloadTextLines |**lines** |Unload text lines |
...
|GetCameraForward |**camera** |Returns the cameras forward vector (normalized) |
|GetCameraUp |**camera** |Returns the cameras up vector (normalized) Note: The up vector might not be perpendicular to the forward vector |
|GetCameraRight |**camera** |Returns the cameras right vector (normalized) |
|CameraMoveForward |**camera**, **distance**, **moveInWorldPlane**=1 |Moves the camera in its forward direction |
|CameraMoveUp |**camera**, **distance** |Moves the camera in its up direction |
|CameraMoveRight |**camera**, **distance**, **moveInWorldPlane**=1 |Moves the camera target in its current right direction |
|CameraMoveToTarget |**camera**, **delta** |Moves the camera position closer/farther to/from the camera target |
|CameraYaw |**camera**, **angle**, **rotateAroundTarget**=0 |Rotates the camera around its up vector Yaw is "looking left and right" If rotateAroundTarget is false, the camera rotates around its position Note: angle must be provided in radians |
|CameraPitch |**camera**, **angle**, **lockView**=1, **rotateAroundTarget**=0, **rotateUp**=0 |Rotates the camera around its right vector, pitch is "looking up and down"  - lockView prevents camera overrotation (aka "somersaults")  - rotateAroundTarget defines if rotation is around target or around its position  - rotateUp rotates the up direction as well (typically only useful in CAMERA_FREE) NOTE: [angle] must be provided in radians |
|CameraRoll |**camera**, **angle** |Rotates the camera around its forward vector Roll is "turning your head sideways to the left or right" Note: angle must be provided in radians |
|GetCameraViewMatrix |**camera** |Returns the camera view matrix |
|GetCameraProjectionMatrix |**camera**, **aspect**=0 |Returns the camera projection matrix |
...
|ExportWave |**wave**, **fileName** |Export wave data to file |
...
|Name | Parameters | Purpose |
|-----|------------|---------|
|updateGeometry |**shapes**, **verts**, **pos**, **bodyMask** |Transform shapes into world coordinates and compute their bounds, for the bodies in bodyMask |
